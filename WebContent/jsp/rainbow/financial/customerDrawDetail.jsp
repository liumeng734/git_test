...
<script type="text/javascript">
	...
	jQuery(function($){
		//加载datagride
		$('#customerDrawDetialTable').datagrid({
			title:'用户提款明细表', 				//标题
			method:'post',
			iconCls:'icon-tip', 			//图标
			...
			onLoadSuccess:function(){$('#customerDrawDetialTable').datagrid('clearSelections');}
		});
		
	});
	//表格查询
	function searchCustomerDrawDetail(){
		//alert("helol kitty!");
		var params = $('#customerDrawDetialTable').datagrid('options').queryParams;
		//先取得 datagrid 的查询参数
		var fields =$('#customerDrawDetailForm').serializeArray(); //自动序列化表单元素为JSON对象
		$.each( fields, function(i, field){
			//alert("["+field.name+":"+field.value+"]");
			params[field.name] = field.value; //设置查询参数
		}); 
		$('#customerDrawDetialTable').datagrid('reload'); //设置好查询参数 reload 一下就可以了
	}
			
	//清空查询条件
	function clearCustomerDrawDetail(){
		$('#customerDrawDetailForm').form('clear');
		//searchUser();
	}
...
<form id="customerDrawDetailForm" style="margin:0; text-align:center;">
	...
	<table style="width:98%; font-size:12px; border:0px solid #f0f0f0;background-color:#EEF2FB" >
		<tr>
			...
			<td width="20%" align="center">
				<a href="#" onclick="clearCustomerDrawDetail();" class="easyui-linkbutton" iconCls="icon-undo">清空</a>
			</td>                        
		</tr>
		<tr>
			...
			<td align="center">
				<a href="#" onclick="searchCustomerDrawDetail();" class="easyui-linkbutton" iconCls="icon-search">查询</a>
			</td>
		</tr>
	</table>
	</div>
...
<div style="padding:5;background-color: #EEF2FB" id="tabdiv">
	<table id="customerDrawDetialTable"></table>
</div>
