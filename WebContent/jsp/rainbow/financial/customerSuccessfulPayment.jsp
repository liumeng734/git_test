...
<script type="text/javascript">
	...
	jQuery(function($){
		//加载datagride
		$('#customerSuccessfulPaymentTable').datagrid({
			title:'提款成功用户一览表', 				//标题
			method:'post',
			iconCls:'icon-tip', 			//图标
			...
			url:"server/queryCustomerDistill?result=" + 3, 	//数据来源
			sortName: 'id',					//排序的列
			sortOrder: 'desc', 				//倒序
			...
			onLoadSuccess:function(){$('#customerSuccessfulPaymentTable').datagrid('clearSelections');}
		});
		
	});
	//表格查询
	function serachCustomerSuccessfulPayment(){
		//alert("helol kitty!");
		var params = $('#customerSuccessfulPaymentTable').datagrid('options').queryParams;
		//先取得 datagrid 的查询参数
		var fields =$('#customerSuccessfulPaymentForm').serializeArray(); //自动序列化表单元素为JSON对象
		$.each( fields, function(i, field){
			//alert("["+field.name+":"+field.value+"]");
			params[field.name] = field.value; //设置查询参数
		}); 
		$('#customerSuccessfulPaymentTable').datagrid('reload'); //设置好查询参数 reload 一下就可以了
	}
			
	//清空查询条件
	function clearCustomerSuccessfulPaymentForm(){
		$('#customerSuccessfulPaymentForm').form('clear');
		//searchUser();
	}
...
<form id="customerSuccessfulPaymentForm" style="margin:0; text-align:center;">
	...
	<table style="width:98%; font-size:12px; border:0px solid #f0f0f0;background-color:#EEF2FB" >
		<tr>
			...
			<td width="20%" align="center">
				<a href="#" onclick="clearCustomerSuccessfulPaymentForm();" class="easyui-linkbutton" iconCls="icon-undo">清空</a>
			</td>                        
		</tr>
		<tr>
			...
			<td align="center">
				<a href="#" onclick="serachCustomerSuccessfulPayment();" class="easyui-linkbutton" iconCls="icon-search">查询</a>
			</td>
		</tr>
	</table>
	</div>
...
<div style="padding:5;background-color: #EEF2FB" id="tabdiv">
	<table id="customerSuccessfulPaymentTable"></table>
</div>
